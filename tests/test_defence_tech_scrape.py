...
def test_defence_tech_scrape():
    print("\n--- Running test scrape for category: Defence Tech ---")
    ...
    try:
        results = scraper_manager.scrape_category("Defence Tech", db)
        db.commit()
        print("\n--- Scrape Results ---")
        ...
        articles = db.query(Article).filter(Article.category == "Defence Tech").order_by(Article.scraped_date.desc()).limit(5).all()
        if articles:
    ...
    print("--- Test scrape for Defence Tech finished ---\n")

if __name__ == "__main__":
    test_defence_tech_scrape()
