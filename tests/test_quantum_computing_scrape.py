...
def test_quantum_computing_scrape():
    print("\n--- Running test scrape for category: Quantum Computing ---")
    ...
    try:
        results = scraper_manager.scrape_category("Quantum Computing", db)
        db.commit()
        print("\n--- Scrape Results ---")
        ...
        articles = db.query(Article).filter(Article.category == "Quantum Computing").order_by(Article.scraped_date.desc()).limit(5).all()
        if articles:
    ...
    print("--- Test scrape for Quantum Computing finished ---\n")

if __name__ == "__main__":
    test_quantum_computing_scrape()
