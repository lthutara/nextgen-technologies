...
def test_renewable_energy_scrape():
    print("\n--- Running test scrape for category: Renewable Energy ---")
    ...
    try:
        results = scraper_manager.scrape_category("Renewable Energy", db)
        db.commit()
        print("\n--- Scrape Results ---")
        ...
        articles = db.query(Article).filter(Article.category == "Renewable Energy").order_by(Article.scraped_date.desc()).limit(5).all()
        if articles:
    ...
    print("--- Test scrape for Renewable Energy finished ---\n")

if __name__ == "__main__":
    test_renewable_energy_scrape()
